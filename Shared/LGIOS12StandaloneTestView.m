...
// The standalone test card is now ONLY the test-surface concerns: the drop
// shadow, the caption, the pan gesture and the drag redraw loop. Every piece
// of Metal and provider logic it used to own moved verbatim into
// LGIOS12MetalGlassView so the Dock can reuse the same verified renderer.
@interface LGIOS12FloatingGlassTestView : LGIOS12MetalGlassView
@end
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (!self.metalInitialized) return self;
...
- (void)dragRedrawDisplayLinkFired:(CADisplayLink *)displayLink {
    (void)displayLink;
    if (self.rendererReady && self.window && !self.hidden && self.alpha > 0.01) {
